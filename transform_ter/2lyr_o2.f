      ...
      integer ispc,nspc
      parameter (ispc=45000000,nspc=18)
      ...
      integer igxi,iget,igze,iGs,iGd,iG1
      integer iw2,iwf,idwf,ipf,inf1,inf2,inf3,iuf,ivf
      integer iu,iv,iw,i1,i2,izsr,izsf,idz,ifld,ift,isgzt,iwgzt
      integer igh,isz(maxslb),iwz(maxslb),nslb,rslb,lslb,ntop
      integer igut,igvt,igus,igvs,icalc,is,ifld1,ifld2
      integer igm,igm1,igm2,igm3,igm4,igm5,ical,icbl,icau,icbu
      integer ica,icb,ic1,ic1d,ic2d,ntopl,ntopu
      integer nx,ny,nz,nzt,nxout,nyout,nzout,itopo,nzl,kout
      real dx,dy,x0,y0,zt,ztop,pi,vel,mix,xout,yout,twod
      real time1,time2,rtmp(2),ztmp
 ...
 886  FORMAT(A80)
      ...
      if (nspc.lt.18) then
        write(6,*)
        write(6,*) 'Error:  need space for at least 18 variables.'
     ...
     >             '        when using mixing.'
      ...
      nzl = nint(a(isclr+12-1)/dz) + 1
      if (nzl.le.1) then
        write(6,*) 'Error:  lower layer depth less than dz.'
        write(6,*) 'Stopping.'
        stop
      end if
      dz = a(isclr+12-1)/real(nzl-1)
 ...
 89   if (k.le.nspc) then
     ...
     >                       nx,ny,nsclr)
      ...
      write(6,*) 'Setting up output file.'
     ...
     >                  nx,ny,nxout,nyout,0,1)
      ...
      write(6,*) 'Done setting up output file.'
      write(6,*) 'Calculating preliminary info (gh, matching ',
     >               'conditions, etc.).'
     ...
     >                                       0,0)
      ...
      call calc_gh(a(igh),a(iu1),a(iv1),a(iter(1)),a(izs_p),
     ...
     >                                       0,0)
      ...
      write(6,*) 'Done preliminary calculations.'
 ...
 91   if (k.le.nzt+2) then
      ...
      write(6,*) 'Calculating radiation condition.'

      rslb = 0
      if (mix.ne.0.) then
        isz(1) = isgzt + nzl+2-6
        iwz(1) = iwgzt + nzl+2-6
 95     if (isz(rslb+1)-isgzt+1.lt.nzt-3) then
          rslb = rslb + 1
     ...
     >                           'calculation!'
        ...
        isz(1) = isgzt + nzl-1
        iwz(1) = iwgzt + nzl-1
 97     if (isz(rslb+1)-isgzt+1.lt.nzt) then
          rslb = rslb + 1
          if (rslb.gt.maxslb-1) then
            write(6,*) 'Error:  too many slabs for radiation ',
     >                           'calculation!'
      ...
      enddo
      ic1 = 0
      call intgrl_coeffs(a,ispc,ic1,icbu,iu1,iv1,iw1,ib1,igu,igv,igb,
     ...
     >                             nx,ny,0,0)
      call rad_bc(a(icbu),a(igut),a(igvt),a(ik),a(il),zt,nx,ny)

      write(6,*) 'Done calculating radiation condition.'
      write(6,*) 'Calculating constant coefficients.'

      isz(1) = isgzt
      iwz(1) = iwgzt
      lslb = 0
      if (mix.ne.0.) then
 99     if (isz(lslb+1)-isgzt+1.lt.nzl-3) then
          lslb = lslb + 1
          isz(lslb+1) = isz(lslb) + nz-6
          iwz(lslb+1) = iwz(lslb) + nz-6
          goto 99
        end if
        ntop = nzl+2 - (isz(lslb)-isgzt+1) + 1
      else
 101    if (isz(lslb+1)-isgzt+1.lt.nzl) then
          lslb = lslb + 1
          isz(lslb+1) = isz(lslb) + nz
          iwz(lslb+1) = iwz(lslb) + nz
          goto 101
        end if
        ntop = nzl - (isz(lslb)-isgzt+1)
      end if
      ...
      do ij=1,nx*ny
        ...
        a(ic2d+ij-1) = 0.
      enddo
      call intgrl_coeffs(a,ispc,ic1d,ic2d,iu1,iv1,iw1,ib1,igu,igv,igb,
     >                   idgu,idgv,id11,id12,id13,id22,id23,id33,ikm,
     >                   fptr1,fptr2,fptr3,izs_p,izs_u,izs_v,itran,
     >                   iwrk,iwrk_x,iwrk_y,ik,il,isclr,isz(1),iwz(1),
     >                   lslb,ntop,dx,dy,dz,nx,ny,nz,0)
      ...
      do ij=1,nx*ny
        ...
        a(icau+ij-1) = 0.
      enddo
      call const_coeffs(a(ical),a(icbl),a(icau),a(icbu),a(igh),
     >              a(ic1d),a(ic2d),a(igm1),a(igm5),a(ik),a(il),
     >              nx,ny)

      do ij=1,nx*ny
        a(igh+ij-1) = a(ical+ij-1)
        a(igm1+ij-1) = a(icbl+ij-1)
        a(igm5+ij-1) = a(icau+ij-1)
      enddo
      ical = igh
      icbl = igm1
      icau = igm5

      write(6,*) 'Done calculating constant coefficients.'
      write(6,*) 'Calculating drag.'

      call drag_calc(a(ical),a(icbl),a(izs_p),a(igus),a(igvs),
     >                a(itran(1)),a(itran(2)),a(itran(3)),
     >                a(iwrk(1)),a(iwrk(2)),a(iwrk(3)),a(iter(1)),
     >                a(iter(2)),a(iter(3)),a(iwrk_x),a(iwrk_y),
     >                a(ik),a(il),dx,dy,nx,ny)

      write(6,*) 'Done calculating drag.'
      write(6,*) 'Initializing integrals.'
     ...
     >                                       0,0)
      ...
      call calc_Gfac(a(iGs),a(idgu),a(idgv),a(igb),a(ik),a(il),
     >                             nx,ny)
      ...
      do k=1,3
        ...
        call calc_o1(a(iptr),a(fptr1),a(izs_p),a(itran(1)),a(iwrk(1)),
     >            ztmp,a(iwrk_x),a(iwrk_y),a(ik),a(il),dx,dy,ctmp,
     >            nx,ny,1,0,0)
      enddo
     ...
     >         a(fptr2),a(fptr3),a(izs_p),a(itran(1)),a(itran(2)),
     >         a(itran(3)),a(iwrk(1)),a(iwrk(2)),a(iwrk(3)),
     >         a(iwrk_x),a(iwrk_y),a(ik),a(il),ztmp,nx,ny,1,0,0,idz)
        call trnsfrm2d(a(iptr),a(itran(1)),a(iwrk_x),a(iwrk_y),nx,ny,
     >                                       0,0)
      enddo

      call calc_Gfac(a(iGd),a(idgu),a(idgv),a(igb),a(ik),a(il),
     >                             nx,ny)

      write(6,*) 'Done initializing integrals.'

      nslb = lslb
      ntopl = ntop
      if (mix.ne.0.) then
        isz(nslb+1) = isgzt + nzl+2-6
        iwz(nslb+1) = iwgzt + nzl+2-6
 103    if (isz(nslb+1)-isgzt+1.lt.nzout-3) then
          nslb = nslb + 1
          isz(nslb+1) = isz(nslb) + nz-6
          iwz(nslb+1) = iwz(nslb) + nz-6
          goto 103
        end if
        ntopu = nzout+2 - (isz(nslb)-isgzt+1) + 1
      else
        isz(nslb+1) = isgzt + nzl-1
        iwz(nslb+1) = iwgzt + nzl-1
 105    if (isz(nslb+1)-isgzt+1.lt.nzout) then
          nslb = nslb + 1
          isz(nslb+1) = isz(nslb) + nz
          iwz(nslb+1) = iwz(nslb) + nz
          goto 105
        end if
        ntopu = nzout - (isz(nslb)-isgzt+1)
      end if
      ...
      write(6,*) 'Starting slab calculations.  nslb = ',nslb

      kout = 1
      is = 1
 107  if (is.le.nslb) then

        write(6,*) 'STARTING SLAB ',is
 
        if (is.eq.lslb) then
          nzs = ntopl
        else if (is.eq.nslb) then
          nzs = ntopu
        else
          nzs = nz
        end if

        if (is.eq.lslb+1) then
          iG1 = iGd
          ica = icau
          icb = icbu
          do ij=1,nx*ny
        ...
        write(6,*) 'Linear calculations and mixing.'
     ...
     >                     a(isclr+23-1),dx,dy,nx,ny,nzs)
          ...
          if (twod.ne.0.) then
            call set_2d(a(idgu),a(idgv),a(igb),a(igu),a(igv),
     ...
     >             nyout,nzout,iout,jout,kout,iu,iv,i1,i2)
          end if
        enddo

        write(6,*) 'Done linear calculations and mixing.'
        write(6,*) 'Calculating advective forcing.'
     ...
     >         0,0,idz)
c         call write_3d(idcdf1,a(iptr),a(fptr),a(iter(1)),a(isz(is)),
c    >           a(isclr+2-1),mix,ctmp,nx,ny,nz,nzs,nxout,nyout,
c    >           nzout,iout,jout,kout,0,0,0,0)     
        enddo
     ...
     >                            nzs,0,0)
        ...
        write(6,*) 'Done advective forcing.'
        write(6,*) 'Caculating second order corrections.'
     ...
     >             nzout,iout,jout,kout,0,0,0,0)
        ...
        write(6,*) 'Done second order corrections.'
        write(6,*) 'DONE SLAB ',is
      ...
      write(6,*) 'Done slab calculations.'
      write(6,*) 'Filling in fields at top.'
     ...
     >                nzout,iout,jout,kout,0,0,0,0)
      ...
      write(6,*) 'Done fields at top.'

      if (itopo.eq.1) then

        write(6,*) 'Setting up second output file.'
     ...
     >                   jout,nx,ny,nxout,nyout,0,1)
        ...
        write(6,*) 'Done second output file.'
        write(6,*) 'Interpolating fields to transformed grid.'
     ...
     >                 isclr,nx,ny,nxout,nyout,nzout,iout,jout)
        ...
        write(6,*) 'Done interpolating fields.'
...
*----------------------------------------------------------------------
      ...
      subroutine intgrl_coeffs(a,ispc,ic1,ic2,iu1,iv1,iw1,ib1,igu,
     >                       igv,igb,idgu,idgv,id11,id12,id13,id22,
     ...
     >                       nx,ny,nz,irad)
      ...
      integer ic1,ic2,iu1,iv1,iw1,ib1,igu,igv,igb,idgu,idgv
      integer id11,id12,id13,id22,id23,id33,ikm,fptr1,fptr2,fptr3
      integer izs_p,izs_u,izs_v,itran(3),iwrk(3),iwrk_x,iwrk_y,ik,il
      integer isclr,nslb,isz(nslb),iwz(nslb),ntop,nx,ny,nz,irad
      real dx,dy,dz

      integer ijk,is,j,k
      integer nzs,iu,iv,iw,igz,idz,iGfac,isave(3)
      real mix,twod
      character*(80) ctmp
 ...
 99   if (is.le.nslb) then
     ...
     >                        nx,ny,nzs,0,0)
        ...
        iGfac = iwrk(1) 
        if (irad.eq.1) ic1 = iwrk(2)

        call calc_intgrl_coeffs(a(ic1),a(ic2),a(idgu),a(idgv),
     >                    a(igb),a(iGfac),a(ik),a(il),a(isz(is)),
     >                    dz,nx,ny,nzs,irad)
...
*---------------------------------------------------------------------
      ...
      subroutine calc_intgrl_coeffs(c1,c2,dgu,dgv,gb,Gfac,ak,al,
     >                                sgz,dz,nx,ny,nz,irad)

      integer nx,ny,nz,irad
      real ak(nx-1),al(ny-1),sgz(nz),dz
      complex c1(nx-1,ny-1),c2(nx-1,ny-1),dgu(nx-1,ny-1,nz)
      complex dgv(nx-1,ny-1,nz),gb(nx-1,ny-1,nz),Gfac(nx-1,ny-1)
       
      real fcor,d,Nu,Nl,U0,V0,ps,rhos,g
      common /sclrblk/ fcor,d,Nu,Nl,U0,V0,ps,rhos,g

      integer i,j,k
      real omga,ml2,mu2,gaml2,gamu2,m2,gam2,m,gam
      complex exfac1,exfac2
      logical mzero2,requ
      ...
      do k=1,nz
        ...
        call calc_Gfac(Gfac,dgu(1,1,k),dgv(1,1,k),gb(1,1,k),
     >                           ak,al,nx,ny)
        ...
        do i=1,nx-1
          ...
          if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
            ...
            c2(i,j) = 0.
            if (irad.eq.0) c1(i,j) = 0.

          else if (m2.gt.0.) then

            m = sqrt(m2)*omga/abs(omga)
            exfac1 = cmplx(cos(m*(tau-d)),-sin(m*(tau-d)))
            exfac2 = cmplx(cos(m*(tau-d)),sin(m*(tau-d)))
            c2(i,j) = c2(i,j) + cmplx(0.,0.5/m)*Gfac(i,j)
     >                               * exfac2*dz
            if (irad.eq.0) then
              c1(i,j) = c1(i,j) + cmplx(0.,0.5/m)*Gfac(i,j)
     >                               * exfac1*dz
            end if
            ...
            gam = sqrt(gam2)
            exfac1 = cmplx(exp(gam*(tau-d)),0.)
            exfac2 = cmplx(exp(-gam*(tau-d)),0.)
            c2(i,j) = c2(i,j) + cmplx(0.5/gam,0.)*Gfac(i,j)
     >                      * exfac2*dz
            if (irad.eq.0) then
              c1(i,j) = c1(i,j) + cmplx(0.5/gam,0.)*Gfac(i,j)
     >                      * exfac1*dz
            end if
...
*----------------------------------------------------------------------
      ...
      subroutine rad_bc(bu,gu,gv,ak,al,zt,nx,ny)

      integer nx,ny
      real ak(nx-1),al(ny-1),zt
      complex bu(nx-1,ny-1),gu(nx-1,ny-1),gv(nx-1,ny-1)

      real fcor,d,Nu,Nl,U0,V0,ps,rhos,g
      common /sclrblk/ fcor,d,Nu,Nl,U0,V0,ps,rhos,g

      integer i,j
      real omga,ml2,mu2,gamu2,mu,gamu,fac(4)
      logical mzero2,requ
      complex exfac
      ...
      do i=1,nx-1
        ...
        if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
          ...
          mu = sqrt(mu2)*omga/abs(omga)
          exfac = cmplx(cos(mu*(zt-d)),sin(mu*(zt-d)))
          fac(1) = 0.5*fcor*al(j)/mu/(omga**2-fcor**2)
          fac(2) = -0.5*omga*ak(i)/mu/(omga**2-fcor**2)
          fac(3) = -0.5*fcor*ak(i)/mu/(omga**2-fcor**2)
          fac(4) = -0.5*omga*al(j)/mu/(omga**2-fcor**2)
          bu(i,j) = bu(i,j) + cmplx(fac(1),fac(2))*gu(i,j)*exfac
     >                 + cmplx(fac(3),fac(4))*gv(i,j)*exfac

        else

          gamu = sqrt(gamu2)
          exfac = cmplx(exp(-gamu*(zt-d)),0.)
          fac(1) = -0.5*omga*ak(i)/gamu/(omga**2-fcor**2)
          fac(2) = -0.5*fcor*al(j)/gamu/(omga**2-fcor**2)
          fac(3) = -0.5*omga*al(j)/gamu/(omga**2-fcor**2)
          fac(4) = 0.5*fcor*ak(i)/gamu/(omga**2-fcor**2)
          bu(i,j) = bu(i,j) + cmplx(fac(1),fac(2))*gu(i,j)*exfac
     >                   + cmplx(fac(3),fac(4))*gv(i,j)*exfac
...
*----------------------------------------------------------------------
      ...
      subroutine const_coeffs(cal,cbl,cau,cbu,gh,c1d,c2d,gm1,gm5,
     >                               ak,al,nx,ny)

      integer nx,ny
      real ak(nx-1),al(ny-1)
      complex cal(nx-1,ny-1),cbl(nx-1,ny-1)
      complex cau(nx-1,ny-1),cbu(nx-1,ny-1)
      complex gh(nx-1,ny-1),c1d(nx-1,ny-1),c2d(nx-1,ny-1)
      complex gm1(nx-1,ny-1),gm5(nx-1,ny-1)

      real fcor,d,Nu,Nl,U0,V0,ps,rhos,g
      common /sclrblk/ fcor,d,Nu,Nl,U0,V0,ps,rhos,g

      real mu,ml,mu2,ml2,gamu,gaml,gamu2,gaml2,omga,R,alph
      logical mzero2,requ
      complex exfac1,exfac2,cfac
      ...
      do i=1,nx-1
        ...
        if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
          ...
          mu = alph*ml
          exfac1 = cmplx(cos(ml*d),sin(ml*d))
          exfac2 = cmplx(cos(ml*d),-sin(ml*d)) 

          cfac = exfac1/(cmplx(R,0.)*exfac1+exfac2)
          cal(i,j) = cfac*( exfac2*gh(i,j) - cmplx(R,0.)*c1d(i,j)
     >              - c2d(i,j) - cmplx(1.-R,0.)*cbu(i,j)
     >                 + cmplx(0.5*(1.-R),0.)*gm1(i,j)
     >               + cmplx(0.,0.5*(1.+R)/ml)*gm5(i,j) )

          cfac = exfac2/(cmplx(R,0.)*exfac1+exfac2)
          cbl(i,j) = cfac*( cmplx(R,0.)*exfac1*gh(i,j)
     >               + cmplx(R,0.)*c1d(i,j) + c2d(i,j) 
     >                  + cmplx(1.-R,0.)*cbu(i,j)
     >               - cmplx(0.5*(1.-R),0.)*gm1(i,j)
     >             - cmplx(0.,0.5*(1.+R)/ml)*gm5(i,j) )

          cfac = cmplx(1.+R,0.)/(cmplx(R,0.)*exfac1+exfac2)
          cau(i,j) = cfac*( gh(i,j) + exfac2*c1d(i,j) 
     >                     - exfac1*c2d(i,j) 
     >      - (cmplx(R,0.)*exfac2+exfac1)/cmplx(1.+R,0.)*cbu(i,j)
     >               + cmplx(cos(ml*d),0.)*gm1(i,j)
     >              - cmplx(sin(ml*d)/ml,0.)*gm5(i,j) )
     ...
     >                     - exfac1*c2d(i,j) 
     >      - (cmplx(R,0.)*exfac2+exfac1)/cmplx(1.+R,0.)*cbu(i,j)
     >             + cmplx(cosh(gaml*d),0.)*gm1(i,j)
     >           - cmplx(sinh(gaml*d)/gaml,0.)*gm5(i,j) )
        end if
...
*----------------------------------------------------------------------
      ...
      subroutine set_2d(dgu,dgv,gb,gu,gv,km,gut,gvt,nx,ny,nz)

      integer nx,ny,nz
      real dgu(nx-1,ny-1,nz),dgv(nx-1,ny-1,nz)
      real gb(nx-1,ny-1,nz),gu(nx-1,ny-1,nz),gv(nx-1,ny-1,nz)
      real km(nx-1,ny-1,nz),gut(nx-1,ny-1),gvt(nx-1,ny-1)
...
*------------------------------------------------------------------------
      ...
      subroutine calc_Gfac(Gfac,dgu,dgv,gb,ak,al,nx,ny)      

      integer nx,ny
      real ak(nx-1),al(ny-1)
      complex Gfac(nx-1,ny-1),dgu(nx-1,ny-1),dgv(nx-1,ny-1)
      complex gb(nx-1,ny-1)
...
*------------------------------------------------------------------------
     ...
     >                       dx,dy,var,nx,ny,nz,iu,iv)
      ...
      integer nx,ny,nz,iu,iv
      real fld(nx-1+iu,ny-1+iv,nz),ak(nx-1),al(ny-1)
      real gz(nz),cwrkx(4*nx+15),cwrky(4*ny+15),dx,dy
      complex ft(nx-1,ny-1,nz),hft(nx-1+iu,ny-1+iv)
      ...
      real alph,fac1,fac2,sfac
      integer nzl,lgth,i,j,k,istp
      logical mzero1,requ
      complex Cl,pfac,pfac1,pfac2,exfac1,exfac2
      ...
      nzl = 0
      k = 1
 234  if ((k.le.nz).and.(istp.eq.0)) then
        if (gz(k).lt.d) then
          nzl = nzl+1
          k = k+1
        else
     ...
     >                       - g*z + 0.5*(Nl**2)*(z**2)
      ...
      do i=1,nx-1
        ...
        if (mzero1(omga,Nu,Nl,fcor)) then
          ...
          exfac1 = cmplx(cos(ml*d),sin(ml*d))
          exfac2 = cmplx(cos(ml*d),-sin(ml*d))
          Cl = cmplx(1.,0.)/(cmplx(R,0.)*exfac1+exfac2)

          if (nzl.gt.0) then
            call calc_pfac(pfac,sfac,ak,al,ml2,ml,gaml2,0.,Nl,omga,
     >                              fcor,var,i,j,nx,ny,0)
            do k=1,nzl     
              z = gz(k)
              exfac1 = cmplx(cos(ml*(z-d)),sin(ml*(z-d)))
              exfac2 = cmplx(cos(ml*(z-d)),-sin(ml*(z-d)))
              ft(i,j,k) = Cl*(exfac1+sfac*cmplx(R,0.)*exfac2)
     >                              * pfac*hft(i,j)
            enddo
          end if

          if (nzl.lt.nz) then
            call calc_pfac(pfac,sfac,ak,al,mu2,mu,gamu2,0.,Nu,omga,
     >                              fcor,var,i,j,nx,ny,0)
            do k=nzl+1,nz
              z = gz(k)
              exfac1 = cmplx(cos(mu*(z-d)),sin(mu*(z-d)))
              ft(i,j,k) = Cl*cmplx(1.+R,0.)*exfac1*pfac*hft(i,j)
            enddo
          end if
          ...
          if (nzl.gt.0) then
            call calc_pfac(pfac,sfac,ak,al,ml2,0.,gaml2,gaml,Nl,omga,
     >                              fcor,var,i,j,nx,ny,0)
            do k=1,nzl    
              z = gz(k)
     ...
     >                              * pfac*hft(i,j)
          ...
          if (nzl.lt.nz) then
            call calc_pfac(pfac,sfac,ak,al,mu2,0.,gamu2,gamu,Nu,omga,
     >                              fcor,var,i,j,nx,ny,0)
            do k=nzl+1,nz
              z = gz(k)
              exfac1 = cmplx(exp(-gamu*(z-d)),0.)
              ft(i,j,k) = Cl*cmplx(1.+R,0.)*exfac1*pfac*hft(i,j)
            enddo
          end if
...
*---------------------------------------------------------------------
      ...
      subroutine calc_pfac(pfac,sfac,ak,al,m2,m,gam2,gam,N,omga,fcor,
     >                               var,i,j,nx,ny,idz)

      integer i,j,nx,ny,idz
      real sfac,ak(nx-1),al(ny-1),m2,m,gam2,gam,N,omga,fcor
      character*(*) var
      complex pfac
      ...
      do i=1,nx-1
        ...
        do j=1,ny-1
          ...
          if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
            ...
            var = 'w'               
            call calc_pfac(pfac,sfac,ak,al,ml2,ml,gaml2,0.,Nl,omga,
     >                              fcor,var,i,j,nx,ny,1)
            tran3(j,i) = Cl*(exfac2+sfac*cmplx(R,0.)*exfac1)
     >                          * pfac*hft(i,j) 
          else 
            gaml = sqrt(gaml2)
            exfac1 = cmplx(exp(-gaml*d),0.)
...
*---------------------------------------------------------------------
     ...
     >                          wrk3,cwrkx,cwrky,ak,al,nx,ny)
      ...
      if (ctmp(1:3).eq.'gm1') then
        var1 = 'w'
        var2 = 'zeta'
      else if (ctmp(1:3).eq.'gm2') then
        var1 = 'p'
        var2 = 'zeta'
      else if (ctmp(1:3).eq.'gm3') then
        var1 = 'u'
        var2 = 'w'
      else if (ctmp(1:3).eq.'gm4') then
        var1 = 'v'
        var2 = 'w'
      else
      ...
      do i=1,nx-1
        ...
        do j=1,ny-1
          ...
          if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
            ...
            call calc_pfac(pfac,sfac,ak,al,ml2,ml,gaml2,0.,Nl,omga,
     >                              fcor,var1,i,j,nx,ny,1)
            tran1(j,i) = Cl*cmplx(1.+sfac*R,0.)*pfac*hft(i,j)

            call calc_pfac(pfac,sfac,ak,al,mu2,mu,gamu2,0.,Nu,omga,
     >                              fcor,var1,i,j,nx,ny,1)
            tran2(j,i) = Cl*cmplx(1.+R,0.)*pfac*hft(i,j)

            call calc_pfac(pfac,sfac,ak,al,mu2,mu,gamu2,0.,Nu,omga,
     >                              fcor,var2,i,j,nx,ny,0)
            tran3(j,i) = Cl*cmplx(1.+R,0.)*pfac*hft(i,j)
...
*---------------------------------------------------------------------
     ...
     >                     nx,ny,nz,iu,iv,idz)
      ...
      integer nzl,i,j,k,trm,lgth,istp
      real dsdx,dsdy,dsdz,du,ds
      real ml,ml2,mu,mu2,gaml,gaml2,gamu,gamu2,omga,fac1,fac2
      real alph,R,z,sfac,sfac1,sfac2
      character*(8) vtmp,ctmp
      logical mzero2,requ
      complex Cl,pfac,exfac,pfac1,pfac2,exfac1,exfac2
 ...
 234  if ((k.le.nz).and.(istp.eq.0)) then
      ...
      do i=1,nx-1
        ...
        if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
          ...
          if (nzl.gt.0) then
            call calc_pfac(pfac,sfac,ak,al,ml2,ml,gaml2,0.,Nl,omga,
     >                              fcor,vtmp,i,j,nx,ny,idz)
            do k=1,nzl
              z = gz(k)
              exfac1 = cmplx(cos(ml*(z-d)),sin(ml*(z-d)))
              exfac2 = cmplx(cos(ml*(z-d)),-sin(ml*(z-d)))
              ft1(i,j,k) = cmplx(0.,ak(i))
     >           * Cl*(exfac1+sfac*cmplx(R,0.)*exfac2)*pfac*hft(i,j)
              ft2(i,j,k) = cmplx(0.,al(j))
     >           * Cl*(exfac1+sfac*cmplx(R,0.)*exfac2)*pfac*hft(i,j)
              ft3(i,j,k) = cmplx(0.,ml)
     >           * Cl*(exfac1-sfac*cmplx(R,0.)*exfac2)*pfac*hft(i,j)
            enddo
          end if
     ...
     >                              fcor,vtmp,i,j,nx,ny,idz)
            do k=nzl+1,nz
              ...
              ft1(i,j,k) = cmplx(0.,ak(i))
     >                        * Cl*cmplx(1.+R,0.)*exfac*pfac*hft(i,j)
              ft2(i,j,k) = cmplx(0.,al(j))
     >                        * Cl*cmplx(1.+R,0.)*exfac*pfac*hft(i,j)
              ft3(i,j,k) = cmplx(0.,mu)
     >                        * Cl*cmplx(1.+R,0.)*exfac*pfac*hft(i,j)
            enddo
          end if
          ...
          do i=1,nx-1
            ...
            if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
              ...
              if (nzl.gt.0) then
                if (trm.eq.1) then
                  ctmp = 'u'
                  call calc_pfac(pfac1,sfac1,ak,al,ml2,ml,gaml2,0.,
     >                             Nl,omga,fcor,ctmp,i,j,nx,ny,1)
                  call calc_pfac(pfac2,sfac2,ak,al,ml2,ml,gaml2,0.,
     >                             Nl,omga,fcor,vtmp,i,j,nx,ny,0)
     ...
     >                             Nl,omga,fcor,ctmp,i,j,nx,ny,1)
                  call calc_pfac(pfac2,sfac2,ak,al,ml2,ml,gaml2,0.,
     >                             Nl,omga,fcor,vtmp,i,j,nx,ny,1)
                end if
                do k=1,nzl
                  z = gz(k)
                  exfac1 = cmplx(cos(ml*(z-d)),sin(ml*(z-d)))
                  exfac2 = cmplx(cos(ml*(z-d)),-sin(ml*(z-d)))
                  ft1(i,j,k) = Cl*(exfac1+sfac1*cmplx(R,0.)*exfac2)
     >                               * pfac1*hft(i,j)
                  ft2(i,j,k) = Cl*(exfac1+sfac2*cmplx(R,0.)*exfac2)
     >                               * pfac2*hft(i,j)
                enddo
              end if
              ...
              if (nzl.lt.nz) then
                if (trm.eq.1) then
                  ctmp = 'u'
                  call calc_pfac(pfac1,sfac1,ak,al,mu2,mu,gamu2,0.,
     >                             Nu,omga,fcor,ctmp,i,j,nx,ny,1)
                  call calc_pfac(pfac2,sfac2,ak,al,mu2,mu,gamu2,0.,
     >                             Nu,omga,fcor,vtmp,i,j,nx,ny,0)
                  pfac2 = cmplx(0.,ak(i))*pfac2
                else if (trm.eq.2) then
                  ctmp = 'v'
                  call calc_pfac(pfac1,sfac1,ak,al,mu2,mu,gamu2,0.,
     ...
     >                             Nu,omga,fcor,ctmp,i,j,nx,ny,1)
                  call calc_pfac(pfac2,sfac2,ak,al,mu2,mu,gamu2,0.,
     >                             Nu,omga,fcor,vtmp,i,j,nx,ny,1)
                end if
                do k=nzl+1,nz
                  z = gz(k)
                  exfac = cmplx(cos(mu*(z-d)),sin(mu*(z-d)))
                  ft1(i,j,k) = Cl*cmplx(1.+R,0.)*exfac*pfac1*hft(i,j)
                  ft2(i,j,k) = Cl*cmplx(1.+R,0.)*exfac*pfac2*hft(i,j)
                enddo
              end if
              ...
              if (nzl.gt.0) then
                if (trm.eq.1) then
                  ctmp = 'u'
                  call calc_pfac(pfac1,sfac1,ak,al,ml2,0.,gaml2,gaml,
     >                             Nl,omga,fcor,ctmp,i,j,nx,ny,1)
                  call calc_pfac(pfac2,sfac2,ak,al,ml2,0.,gaml2,gaml,
     >                             Nl,omga,fcor,vtmp,i,j,nx,ny,0)
                  pfac2 = cmplx(0.,ak(i))*pfac2
                else if (trm.eq.2) then
                  ctmp = 'v'
     ...
     >                               * pfac2*hft(i,j)
              ...
              if (nzl.lt.nz) then
                if (trm.eq.1) then
                  ctmp = 'u'
                  call calc_pfac(pfac1,sfac1,ak,al,mu2,0.,gamu2,gamu,
     >                             Nu,omga,fcor,ctmp,i,j,nx,ny,1)
                  call calc_pfac(pfac2,sfac2,ak,al,mu2,0.,gamu2,gamu,
     >                             Nu,omga,fcor,vtmp,i,j,nx,ny,0)
                  pfac2 = cmplx(0.,ak(i))*pfac2
                else if (trm.eq.2) then
                  ctmp = 'v'
...
*----------------------------------------------------------------------
      ...
      subroutine calc_w2(w2,wf,dwf,dgu,dgv,gb,ca,cb,G1,c1,c2,c1w,
     >                      c2w,tran,wrk,Gfac,cwrkx,cwrky,ak,al,
     >                      sgz,wgz,dz,nx,ny,nz,is,iup)

      integer nx,ny,nz,is,iup
      ...
      complex gb(nx-1,ny-1,nz),ca(nx-1,ny-1),cb(nx-1,ny-1)
      complex G1(nx-1,ny-1),c1(nx-1,ny-1),c2(nx-1,ny-1)
      complex c1w(nx-1,ny-1),c2w(nx-1,ny-1)
      complex tran(ny-1,nx-1),wrk(nx-1,ny-1),Gfac(nz)

      real fcor,d,Nu,Nl,U0,V0,ps,rhos,g
      common /sclrblk/ fcor,d,Nu,Nl,U0,V0,ps,rhos,g

      real ml2,mu2,m,m2,gam,gam2,omga,z,tau,dz1
     ...
     >                + fac(3)*gb(i,j,k)) / (omga**2-fcor**2)
        ...
        if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
          ...
          do k=1,nz
            ...
            z = wgz(k)
            exfac1 = cmplx(cos(m*(z-d)),sin(m*(z-d)))
            exfac2 = cmplx(cos(m*(z-d)),-sin(m*(z-d)))

            wf(i,j,k) = (ca(i,j)+c1w(i,j))*exfac1
     >                     + (cb(i,j)-c2w(i,j))*exfac2

            tau = wgz(k) + 0.5*dz
            exfac3 = cmplx(cos(m*(tau-d)),sin(m*(tau-d)))
            exfac4 = cmplx(cos(m*(tau-d)),-sin(m*(tau-d))) 
     ...
     >                          * exfac3*dz
          ...
          do k=1,nz
            ...
            z = wgz(k)
            exfac1 = cmplx(exp(-gam*(z-d)),0.)
            exfac2 = cmplx(exp(gam*(z-d)),0.)

            wf(i,j,k) = (ca(i,j)+c1w(i,j))*exfac1
     >                     + (cb(i,j)-c2w(i,j))*exfac2

            tau = wgz(k) + 0.5*dz
            exfac3 = cmplx(exp(-gam*(tau-d)),0.)
            exfac4 = cmplx(exp(gam*(tau-d)),0.)
     ...
     >                + fac(3)*gb(i,j,k)) / (omga**2-fcor**2)
        ...
        if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
          ...
          Gave = 0.5*(G1(i,j)+Gfac(1))
          exfac3 = cmplx(cos(m*(tau-d)),sin(m*(tau-d)))
          exfac4 = cmplx(cos(m*(tau-d)),-sin(m*(tau-d)))
          c1(i,j) = c1(i,j) + cmplx(0.,0.5/m)*Gave*exfac4*dz1
          c2(i,j) = c2(i,j) + cmplx(0.,0.5/m)*Gave*exfac3*dz1
          
          wf(i,j,1) = (ca(i,j)+c1(i,j))*exfac1
     >                    + (cb(i,j)-c2(i,j))*exfac2
          dwf(i,j,1) = cmplx(0.,m)*(ca(i,j)+c1(i,j))*exfac1
     >                    + cmplx(0.,-m)*(cb(i,j)-c2(i,j))*exfac2
     ...
     >                       * (sgz(k)-sgz(k-1))
            ...
            wf(i,j,k) = (ca(i,j)+c1(i,j))*exfac1
     >                     + (cb(i,j)-c2(i,j))*exfac2
            dwf(i,j,k) = cmplx(0.,m)*(ca(i,j)+c1(i,j))*exfac1
     >               + cmplx(0.,-m)*(cb(i,j)-c2(i,j))*exfac2 
          ...
          wf(i,j,1) = (ca(i,j)+c1(i,j))*exfac1
     >                    + (cb(i,j)-c2(i,j))*exfac2
          dwf(i,j,1) = cmplx(-gam,0.)*(ca(i,j)+c1(i,j))*exfac1
     >                + cmplx(gam,0.)*(cb(i,j)-c2(i,j))*exfac2
     ...
     >                         * (sgz(k)-sgz(k-1))
            ...
            wf(i,j,k) = (ca(i,j)+c1(i,j))*exfac1 
     >                       + (cb(i,j)-c2(i,j))*exfac2
            dwf(i,j,k) = cmplx(-gam,0.)*(ca(i,j)+c1(i,j))*exfac1
     >                   + cmplx(gam,0.)*(cb(i,j)-c2(i,j))*exfac2 
...
*------------------------------------------------------------------------
     ...
     >                        cwrky,ak,al,nx,ny,nz,iup)

      integer nx,ny,nz,iup
...
*------------------------------------------------------------------------
      ...
      subroutine drag_calc(cal,cbl,hft,gus,gvs,p2,dwdz,tran1,   
     ...
     >                      cwrky,ak,al,dx,dy,nx,ny) 
      ...
      real p2(nx-1,ny-1)
      complex cal(nx-1,ny-1),cbl(nx-1,ny-1),hft(nx-1,ny-1)
      ...
      real ml,gaml,ml2,mu2,gaml2,omga,fac,sum1x,sum1y,sum2x,sum2y
      real p1,dp1dz,R,alph
      character*(80) ctmp
      logical requ,mzero2
      complex Cl,exfac1,exfac2,pfac1,pfac2 
      ...
      do i=1,nx-1
        ...
        if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
          ...
          exfac1 = cmplx(cos(ml*d),-sin(ml*d))
          exfac2 = cmplx(cos(ml*d),sin(ml*d))
          dwdz(i,j) = cmplx(0.,ml)*cal(i,j)*exfac1
     >                 + cmplx(0.,-ml)*cbl(i,j)*exfac2
        else
          gaml = sqrt(gaml2)
          exfac1 = cmplx(exp(gaml*d),0.)
          exfac2 = cmplx(exp(-gaml*d),0.) 
          dwdz(i,j) = cmplx(-gaml,0.)*cal(i,j)*exfac1
     >                 + cmplx(gaml,0.)*cbl(i,j)*exfac2
        end if
      ...
      do i=1,nx-1
        ...
        do j=1,ny-1 
          ...
          if (mzero2(omga,Nu,Nl,fcor,ml2,mu2)) then
            ...
            exfac1 = cmplx(cos(ml*d),-sin(ml*d))
            exfac2 = cmplx(cos(ml*d),sin(ml*d))
            Cl = cmplx(1.,0.)/(cmplx(R,0.)*exfac2+exfac1)
            pfac1 = cmplx(0.,Nl**2/ml)*Cl
     >                * (exfac1-cmplx(R,0.)*exfac2) 
            pfac2 = cmplx(-Nl**2,0.)*Cl
     >                * (exfac1+cmplx(R,0.)*exfac2)
...
*----------------------------------------------------------------------
     ...
     >          + aved12**2 + aved13**2 + aved23**2
          ...
          N2 = Nl**2 + (b1(i,j,k+1)-b1(i,j,km1))/(sgz(k+1)-sgz(km1))
...
*----------------------------------------------------------------------
      ...
      subroutine b_mix(km,b1,gb,sgz,wgz,rprandl,dx,dy,nx,ny,nz)

      integer nx,ny,nz
      real km(nx-1,ny-1,nz),b1(nx-1,ny-1,nz),gb(nx-1,ny-1,nz)
      real sgz(nz),wgz(nz),rprandl,dx,dy
...
*---------------------------------------------------------------------
 ...
 789      if ((kplus.eq.0).and.(kptr.le.nzout-1+iw)) then
          ...
          if (kplus.eq.0) kplus = nzout-1+iw

          if (kplus.eq.1) then
            fld2(i,j,k) = fld1(i,j,1) + (fld1(i,j,2)-fld1(i,j,1))
     >                      / (gz(2)-gz(1))*(z-gz(1))
          else
            fld2(i,j,k) = fld1(i,j,kplus) 
     >                + (fld1(i,j,kplus)-fld1(i,j,kplus-1))
     >                    / (gz(kplus)-gz(kplus-1))*(z-gz(kplus))
          end if
...
*----------------------------------------------------------------------
     ...
     >                  asclr,tgrid,sclr,var,units,outfl,nsclr,nvar)
...
*------------------------------------------------------------------------
     ...
     >                           kout,iu,iv,iu2,iv2)
      ...
      integer idcdf,nx,ny,nz,nzs,nxout,nyout,nzout,iout,jout,kout
