/*
 * $Id: nps.c,v 1.10 2001-04-11 18:17:24 harryive Exp $
 *
 ...
 *
 * $Log: nps.c,v $
 * Revision 1.10  2001-04-11 18:17:24  harryive
 * Change getvar call in new_field to read_var to avoid infinite loop.
 *
 * Revision 1.9  2001/03/15 19:09:16  harry
 * Remove extraneous &.
 *
 * Revision 1.8  2001/03/15 18:58:23  harry
 * Fix length of field_data.
 *
 * Revision 1.7  1997/12/18 20:33:18  harry
 * Add field_delta.
 *
 * Revision 1.6  1997/11/10 20:49:40  harry
 * Add Mercator projection.
 *
 * Revision 1.5  1997/11/05 22:54:32  harry
 * Remove data_domain_units_orig.
 *
 * Revision 1.4  1997/10/17 19:38:40  harry
 * Fix lonlat_2_phys transform to handle longitudes above 180.
 *
 * Revision 1.3  1997/10/08 21:45:37  harry
 * Fixed for avn-x and new polar stereographic grids.
 *
 * Revision 1.2  1997/03/20 22:16:12  harry
 * Eliminate more meaningless variables.
 *
 * Revision 1.1  1997/02/19 19:43:14  harry
 * Create a version of the NPS transforms for the new UPC files.
 *
...
static char ident[] = "@(#)$Id: nps.c,v 1.10 2001-04-11 18:17:24 harryive Exp $";
...
static char *ignore[] = {"reftime", "valtime", "datetime", "valtime_offset",
			 "lpdg_bot", "lpdg_top", "fhg", "sigma", "model_id", 
			 "nav_model", "grid_type_code", "grid_type",
			 "grid_name", "grid_center", "grid_number",
			 "earth_shape", "x_dim", "y_dim", "Nx", "Ny",
			 "La1", "Lo1", "ResCompFlag", "Lov", "Dx", "Dy",
			 "ProjFlag", "Latin1", "Latin2", "SpLat", "SpLon",
			 "LI_ls", "LI4_ls", "PRECIP_accum_times",
			 "precip_cn_accum_times", "u_fhg", "v_fhg",
			 "RH_fhg", "T_fhg", "cin_ls", "cape_ls",
			 "RH_ls", "u_ls", "v_ls", "T_ls", 
			 "ls_bot", "ls_top", "ls_bndry_bot",
			 "ls_bndry_top", "ls_sfc_bot", "ls_sfc_top",
			 "ls_all_bot", "ls_all_top", "lfhg_bot",
			 "lfhg_top", "helc_lfhg",
			 NULL};

static int   frtime_size,	/* Size of frtime array */
             grid_type_code,	/* GRIB-1 data type */
             remap_long,	/* Do we have to remap the longitude */
             polar_stereo,	/* Is this a polar stereographic grid? */
	     mercator,		/* Is this a mercator grid? */
...
static float polex, poley,	/* Pole points */
             ...
             cosltn;		/* cos(Latin1) */
/*
 * Variables needed for Mercator.
 */
static float Lo1, Lo2, La1, La2,/* Lat-lon limits */
             Lo_delta,		/* Longitude delta */
             TanLa_start,	/* tangent of latitude start point */
             TanLa_delta;	/* tangent latitude delta */
...

{
    ...
    char data_units[81], display_units[81], **p,
...
/*
  ...
  */
    ...
    /*
      ...
      */
    for (p=ignore; *p != NULL; p++)
	set_button_name_(*p, "", strlen(*p), 0);
    /*
      Get Grid Type.
      */
    if ((data=(float *)getvar_("grid_type_code", &ndims, dims,
			      stagger, min, max, &missing,
			      data_units, display_units,
			      dim_names, &flag,
			      14, 80, 80, 80)) == NULL)
	grid_type_code = -1;
    else grid_type_code = data[0];
    /*
      Get forecast times.
      */
    flag = 0;
    if ((frtime=(float* )getvar_("valtime_offset", &ndims, dims, stagger,
	...
	cdf_dimvar_.data[3] = frtime;
	cdf_dimvar_.size[3] = frtime_size;
	...
	if ((data=(float *)getvar_("lon", &ndims, dims, stagger,
	    ...
	    cdf_dimvar_.data[0] = data;
	    cdf_dimvar_.size[0] = dims[0];
	}
	flag = 0;
	if ((data=(float *)getvar_("lat", &ndims, dims, stagger,
	    ...
	    cdf_dimvar_.data[1] = data;
	    cdf_dimvar_.size[1] = dims[0];
	}
	plmin[2] = 1.;
	plmax[2] = 1.;
	domain_units[2][0] = '\0';
    ...
    /*
      Do we adjust the longitudes (avn-x)?
    */
    if (grid_type_code==0 && cdf_dimvar_.size[0]>1 && plmin[0] == plmax[0]) {
	remap_long = 1;
	plmax[0] += 360;
	for (i=1; i < cdf_dimvar_.size[0]; i++)
	    if (cdf_dimvar_.data[0][i] < cdf_dimvar_.data[0][i-1])
		cdf_dimvar_.data[0][i] += 360;
    }
    else remap_long = 0;
    for (i=0; i < 3; ++i) {
    ...
    setrarr_("phmin", plmin, &four, &error, 5);
    setrarr_("phmax", plmax, &four, &error, 5);
    /*
	...
	polar_stereo = 1;
	orientation[0] = 90.;
	data = (float *)getvar_("Lov", &ndims, dims, stagger,
			      min, max, &missing, data_units,
			      display_units, dim_names, &flag,
			      3, 80, 80, 80);
	orientation[1] = data[0];
	data = (float *)getvar_("Dx", &ndims, dims, stagger,
			      min, max, &missing, data_units,
			      display_units, dim_names, &flag,
			      2, 80, 80, 80);
	grid_increment = data[0];	
	if (convert_units_(data_units, "km", &slope, &intercept,
			   strlen(data_units), 2) == 0) {
	    grid_increment = grid_increment*slope + intercept;
	}
	data = (float *)getvar_("La1", &ndims, dims, stagger,
			      min, max, &missing, data_units,
			      display_units, dim_names, &flag,
			      3, 80, 80, 80);
	La1 = data[0];
	if (strcmp(data_units, "degrees_south") == 0)
	    La1 = -La1;
	data = (float *)getvar_("Lo1", &ndims, dims, stagger,
			      min, max, &missing, data_units,
			      display_units, dim_names, &flag,
			      3, 80, 80, 80);
	Lo1 = data[0];
	if (strcmp(data_units, "degrees_west") == 0)
	    Lo1 = -Lo1;
	pole_point[0] = pole_point[1] = 0;
	lonlat_2_phys_(&x, &y, &Lo1, &La1, &one);
	pole_point[0] = 1.-x;
	pole_point[1] = 1.-y;
    }
    /*
	...
	data = (float *)getvar_("Latin1", &ndims, dims, stagger,
			      min, max, &missing, data_units,
			      display_units, dim_names, &flag,
			      6, 80, 80, 80);
	Latin1 = data[0];
    ...
    /*
      Get grid orientation for map transforms if this is a Mercator grid.
      */
    if ((mercator = (grid_type_code==1))) {
	float Ni, Nj;

	data = (float *)getvar_("Ni", &ndims, dims, stagger,
				min, max, &missing, data_units,
				display_units, dim_names, &flag,
				3, 80, 80, 80);
	Ni = data[0];
	data = (float *)getvar_("Nj", &ndims, dims, stagger,
				min, max, &missing, data_units,
				display_units, dim_names, &flag,
				3, 80, 80, 80);
	Nj = data[0];
	data = (float *)getvar_("Lo1", &ndims, dims, stagger,
				...
				3, 80, 80, 80);
	Lo1 = data[0];
	data = (float *)getvar_("Lo2", &ndims, dims, stagger,
				min, max, &missing, data_units,
				display_units, dim_names, &flag,
				3, 80, 80, 80);
	Lo2 = data[0];
	if (Lo1 > 0 && Lo2 < 0) Lo_delta = (360+Lo2-Lo1)/Ni;
	else Lo_delta = (Lo2 - Lo1) / Ni;
	data = (float *)getvar_("La1", &ndims, dims, stagger,
				min, max, &missing, data_units,
				display_units, dim_names, &flag,
				3, 80, 80, 80);
	La1 = data[0];
	TanLa_start = tan(DRC*La1);
	data = (float *)getvar_("La2", &ndims, dims, stagger,
				min, max, &missing, data_units,
				display_units, dim_names, &flag,
				3, 80, 80, 80);
	La2 = data[0];
	TanLa_delta = (tan(DRC*La2) - TanLa_start) / Nj;
    }
...

{
    ...
    /*
       ...
       */
    if (read_from_nc && frtime_sorted && strcmp(name, "vattime_offset") != 0 &&
	strcmp(dnames[ndims-1], "record")==0) {
	size = inmax[0];
    ...
    /*
       ...
       */
    if (ndims == 3 && strcmp(dnames[2], "record") == 0) {
	inmax[3] = inmax[2];
	inmax[2] = 1;
	*inmax4 = *inmax3;
	*inmax3 = 1;
	...
	if (frtime_sorted) {
	    ...
	    phmin[3] = frtime[0];
	    phmax[3] = frtime[frtime_size-1];
	}
    }
    if (remap_long && ndims > 2 && strcmp(dnames[0], "lon")) {
	phmax[0] += 360;
	do_long = 1;
    }
    else do_long = 0;
	...
	if (inmax[i] != 1) {
	    ...
	    /*
	       ...
	       If such a variable does not exist, coord[i] == NULL.
                   */
	    if (strcmp(dnames[i], "record") == 0) {
		if (frtime_sorted) coord[i] = frtime_sorted;
		else coord[i] = frtime;
		nvalues[i] = frtime_size;
	    }
            else coord[i] =
		(float *)read_var_(dnames[i], &nd, dims, stagger,
				   min, max, &missing,
				   data_units, display_units, dim_names,
				   strlen(dnames[i]), 80, 80, 80);
	    if (coord[i]) {
		values[i] = coord[i];
		nvalues[i] = inmax[i];
		if (i == 2) {
	...
	setrarr_("field_delta", delta, &four, &error, 11);
    }
    set_slicer_values_(values, nvalues);
    for (i=0; i < ndims; i++) window_callback(NULL, i, NULL);
}
...

{
	...
	for(j=0; j<*ndims; j++, idx++) {
		...
		if (inmax[j] == 1) index[idx] = 1.;
			...
			else if (p == phmin[j]) index[idx] = 1;
			else if (p == phmax[j]) index[idx] = inmax[j];
			else if (p > phmax[j]) index[idx] = -1;
			else {
...
int *npts;
{
	...
	for (i=0; i < *npts; ++i) {
	    lon[i] = Lo1 + (x[i]-1)*Lo_delta;
	    if (lon[i] > 180) lon[i] -=360.;
	    lat[i] = RDC*atan(TanLa_start + (y[i]-1)*TanLa_delta);
	}
    }
    else {
	for (i=0; i < *npts; ++i) {
...
int *npts;
{
	...
	for (i=0; i < *npts; ++i) {
	    if (Lo1 > 0 && lon[i] < 0) x[i] = 1. + (360+lon[i]-Lo1)/Lo_delta;
	    else x[i] = 1. + (lon[i]-Lo1)/Lo_delta;
	    y[i] = 1. + (tan(DRC*lat[i])-TanLa_start)/TanLa_delta;
	}
    }
    else {
	for (i=0; i < *npts; ++i) {
